...
/**
  ...
  */
class ICombinMemBridge extends BaseIMemBridge {
  // When the pipeline is supplying a high valid signal
  when (io.cpu_side_io.valid) {
    val request = Wire(new Request)
    request.address   := io.cpu_side_io.address
    request.operation := Read
    request.writedata := 0.U
    request.dirty     := false.B
    request.age       := 0.U

    io.mem_side_io.request.bits  := request
  ...
  io.cpu_side_io.ready := true.B

  // When the memory is outputting a valid instruction
  io.cpu_side_io.good := true.B
  io.cpu_side_io.instruction := io.mem_side_io.response.bits.data
}
...
/**
  ...
  */
class DCombinMemBridge extends BaseDMemBridge {
  io.cpu_side_io.good := true.B

  when (io.cpu_side_io.valid && (io.cpu_side_io.memread || io.cpu_side_io.memwrite)) {
    // Check that we are not issuing a read and write at the same time
    assert(!(io.cpu_side_io.memread && io.cpu_side_io.memwrite))

    io.mem_side_io.request.bits.address := io.cpu_side_io.address
    io.mem_side_io.request.valid := true.B

    when (io.cpu_side_io.memwrite) {
      ...
      io.mem_side_io.request.bits.operation := ReadWrite
    } .otherwise {
      // Issue a normal read to the backing memory
      io.mem_side_io.request.bits.operation := Read
      // Set the dirty bit and the age bits, should not be used in combinational memory mode, which doesn't allow cache system
      io.mem_side_io.request.bits.dirty := false.B
  ...
  when (io.mem_side_io.response.valid) {
    when (io.cpu_side_io.memwrite) {
      ...
      when (io.cpu_side_io.maskmode =/= 3.U) {
        ...
        when (io.cpu_side_io.maskmode === 0.U) { // Byte
          writedata_mask := Cat(Fill(56, 0.U(1.W)), Fill(8, 1.U(1.W)))
        } .elsewhen (io.cpu_side_io.maskmode === 1.U) { // Half-word
          writedata_mask := Cat(Fill(48, 0.U(1.W)), Fill(16, 1.U(1.W)))
        } .elsewhen (io.cpu_side_io.maskmode === 2.U) { // Word
        ...
        writedata_shifted := io.cpu_side_io.writedata << (offset * 8.U)
        ...
        writedata := io.cpu_side_io.writedata
      }

      io.mem_side_io.request.bits.writedata := writedata
    } .elsewhen (io.cpu_side_io.memread) {
